...
df.drop('Gênero', inplace=True, axis=1)
df.drop('Região', inplace=True, axis=1)
df.drop('TipoBatida', inplace=True, axis=1)
...
mds = MDS(dissimilarity='precomputed', random_state=None)
# Get the embeddings
...
plt.savefig('Imagens2/posicoesOriginais.png')
plt.close()
...
plt.savefig('Imagens2/grafico2deuclides.png')
plt.close()
...
plt.title('Gráfico de duas dimensões')
fig.subplots_adjust(wspace=.4, hspace=0.5)
plt.savefig('Imagens2/grafico2dmanhattan.png')
plt.close()
...
for dim in range(1, max_range):
    ...
    mds = MDS(n_components=dim, dissimilarity='precomputed', random_state=None)
    # Apply MDS
    pts = mds.fit_transform(dist_euclid)
...
plt.ylabel('Estresse')
plt.savefig('Imagens2/stress.png')
plt.close()